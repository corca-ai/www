...
`pnpm check:performance-contract`; that build step is CI-only, so run all three
locally when you want the closest preview of the `quality` job. A
`duplication-and-docs` job installs nose and
