...
- sitemap에는 canonical 공개 페이지와 블로그 글만 포함한다. 블로그의
  `?topic=`·`?q=` 필터 상태는 locale별 블로그 목록으로 canonical되며
  sitemap에 넣지 않는다. 블로그 목록 title과 description은 locale별로
  고유하게 유지한다.
...
- Hero CTA는 crawlable `href="#request"` 링크다. 한국어에서는
  ...
  입력칸을 focus한다. 자동 focus된 빈 성함 입력칸은 첫 입력 전까지 연한
  노란 안내 상태로 보이며 오류 상태가 아니다.
...
- Microsoft Clarity 프로젝트 `xuw7puf7d6`는 공통 head에서 로드되어
