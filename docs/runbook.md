...
Every page is generated by the single `src/pages/[...slug].astro` route. Add an
entry to its route list with an `id`, url `slug` and `basePath`, map that `id` to
a component in the same file, and author the component under
`src/components/pages/`. Its SEO title and description come from
`src/i18n/pageMeta.ts` under the matching key.
