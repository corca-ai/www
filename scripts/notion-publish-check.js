...
second preserved code-like line</pre>
  ...
  assert.match(
    blogStyles,
    /\.post-card img\[src\$="corca-openai-partner-cover\.png"\]\s*\{[^}]*aspect-ratio:\s*16\s*\/\s*9;[^}]*background:\s*#ffffff;[^}]*object-fit:\s*cover;[^}]*object-position:\s*18%\s+center;[^}]*outline:\s*none;/s,
  );
  assert.match(
